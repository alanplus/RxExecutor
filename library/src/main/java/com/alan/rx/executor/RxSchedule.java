...
public class RxSchedule {
    ...
    public static <T> Disposable schedule(final RxScheduleRunnable<T> scheduleRunnable) {
        ...
        }).compose(RxSchedulers.threadMain()).doOnNext(new Consumer<Object>() {
            @Override
            public void accept(Object o) throws Exception {
                try {
                    if (o instanceof RxCache) {
                        RxCache<T> cache = (RxCache<T>) o;
                        scheduleRunnable.scheduleOnMainThread(cache.get());
                    }
